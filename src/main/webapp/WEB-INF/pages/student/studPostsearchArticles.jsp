...
function myProfile(){
	...
	$('#searchArticleform').attr('action', "/ocm/student/getMyProfile").submit();
}
...
function postArticle(){
	var studentId = document.getElementById("studentId").value;
	$('#studentId').val(studentId);
	$('#searchArticleform').attr('action', "/ocm/student/getPostArticle").submit();
}
function displayNextPage(index){
	$('#pageId').val(index);
	$('#searchArticleform').attr('action', "/ocm/student/postSearchArt").submit();
}
function findArticles(){
	$('#searchArticleform').attr('action', "/ocm/student/findArticles").submit();
}
function viewArticle(artId){
	$('#artId').val(artId);
	$('#searchArticleform').attr('action', "/ocm/student/viewArticle").submit();
}
...
<div class="sidebar" data-color="black" data-image="<c:url value="/external/assets/img/sidebar-5.jpg" />">
                ...
                <li class="active">
                    <a id="mngUsersLink" href="#" onclick="findArticles();">
                        <i class="pe-7s-news-paper"></i>
                        <p>Find Articles</p>
                    </a>
                </li>
								...
								<div class="panel panel-default panel-table">
									...
									<div class="panel-body" id="search_table">
										<table class="table table-striped table-list">
											...
											<tbody>
											<c:forEach items="${page.content}" var="art" varStatus="loop">
												<tr>
													<td>${art.articleId}</td>
													<td>${art.heading}</td>
